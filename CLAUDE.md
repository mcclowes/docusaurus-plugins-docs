...
Production serving is wired for Vercel and deployed at `https://docusaurus-plugins.mcclowes.com/mcp`. `api/mcp.ts` is a serverless function that imports `build/mcp/snapshot.json` and serves it via `createNodeHandler`, and `vercel.json` rewrites `/mcp` to `/api/mcp`. Vercel runs the build before tracing the function, so the snapshot exists at bundle time; `build/` is gitignored and never committed. Redeploy (`vercel deploy --prod`) to refresh the snapshot.

The function build logs three non-fatal TypeScript errors (TS1479/TS2732/TS2856): the plugin is ESM-only and the JSON import uses an import attribute, but Vercel type-checks `api/mcp.ts` as CommonJS since the project has no `"type": "module"`. They're harmless — Vercel bundles the function with esbuild, which handles both, and the endpoint serves correctly. Don't "fix" by renaming to `.mts` (Vercel stops registering it as a function and `/api/mcp` 404s) or by adding `"type": "module"` (risks the Docusaurus build). Leave it.
